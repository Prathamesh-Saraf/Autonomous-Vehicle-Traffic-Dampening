...
class Agent:
    ...
    def update(self, agents, radius_track):
        ...
        for i, agent in enumerate(agents):
            if agent != self:
                ...
                # FollowerStopper control
                safe_distance = 2 * math.pi * radius_track / len(agents)
                ...
                if distance < safe_distance:
                    ...
                    # Follower control
                    k_follower = 0.3
                    desired_speed = min(agent.speed, self.speed + k_follower * (distance - safe_distance))

                # PD control for position consensus
                k_p_position = 0.1  # Proportional gain for position consensus
                k_d_position = 0.01  # Derivative gain for position consensus
        ...
        self.speed = min(max(desired_speed, -0.03), 0.03)  # Saturation limits
...
while running:
    ...
    # Update agents
    radius_track = min(screen_width // 2, screen_height // 2) - agents[0].radius - 10
    for agent in agents:
        agent.update(agents, radius_track)
